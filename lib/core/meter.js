...
export function meterTotals(entries) {
    const t = { requests: 0, forwarded: 0, faults: 0, hit: 0, miss: 0, output: 0, reasoning: 0, servedModels: [], fingerprints: [], replayed: 0, harnessIdentities: [] };
...
export function parseResponseBody(body, stream) {
    const out = { usage: null, model: null, fingerprint: null };
    const take = (j) => {
        if (j.usage)
            out.usage = normalizeUsage(j.usage) ?? out.usage;
        if (typeof j.model === 'string' && j.model)
            out.model = j.model;
        if (typeof j.system_fingerprint === 'string' && j.system_fingerprint)
            out.fingerprint = j.system_fingerprint;
    };
    if (!stream) {
        try {
            ...
            const up = make({ protocol: upstream.protocol, hostname: upstream.hostname, port: upstream.port || undefined, path: upstream.pathname.replace(/\/$/, '') + path, method, headers }, (upRes) => {
                ...
                upRes.on('end', () => {
                    ...
                    if (options.recordFile)
                        appendFileSync(options.recordFile, JSON.stringify({ seq: entries.length + 1, requestSha, status: upRes.statusCode ?? 0, contentType: String(upRes.headers['content-type'] ?? 'application/json'), body: text }) + '\n');
