 ...
 */
export function receiptSignatureValid(receipt, publicKeyPem = receipt.publicKey) {
    const { signature, ...rest } = receipt;
    try {
        return verifySignature(null, Buffer.from(canonicalJson(rest)), createPublicKey(publicKeyPem), Buffer.from(signature, 'base64'));
    }
    catch {
        return false;
    }
...
export function keyFingerprint(publicKeyPem) {
    try {
        return createHash('sha256').update(createPublicKey(publicKeyPem).export({ type: 'spki', format: 'der' })).digest('hex').slice(0, 16);
    }
    catch {
        return 'invalid-key';
    }
...
export function sameKey(a, b) {
    try {
        return createPublicKey(a).export({ type: 'spki', format: 'der' }).equals(createPublicKey(b).export({ type: 'spki', format: 'der' }));
    }
    catch {
        return false;
    }
