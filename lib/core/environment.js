...
export function taskContainerArgs(input, options) {
    ...
    for (const [path, mode] of dshRuntimeMounts(input, options.dsh))
...
export async function verifyInEnvironment(environment, testsDir, timeoutMs) {
    ...
    if (reward === null || !Number.isFinite(reward))
        return { ok: false, detail: `verifier wrote no reward${r.code === 124 ? ' (timed out)' : ''}: ${tail || r.stderr.trim().slice(-400)}`, reward: null };
