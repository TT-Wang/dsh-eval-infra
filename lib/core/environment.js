...
export const NODE_SHA256 = { x64: 'd60acfe00a2932254bb0ad20e01b0d74397a0875595de719654b214f4b03f307', arm64: 'fff4078c5def658577f92c88db7db3bc0072924bfb93fe52c1e744a54e94abb8' };
export const NODE_MOUNT = '/opt/dsh-node';
function run(cmd, args, timeoutMs = 120_000, env) {
...
export async function ensureNodeRuntime(evalHome, platform, log) {
    ...
    if (got !== NODE_SHA256[arch]) {
        rmSync(dir, { recursive: true, force: true });
        throw new Error(`${url} does not match its pinned sha256 (got ${got.slice(0, 16)}…, expected ${NODE_SHA256[arch].slice(0, 16)}…); refusing to mount it`);
    }
...
export function taskContainerMounts(input, options) {
    if (options.runtime === undefined)
        return [];
    const out = new Set([NODE_MOUNT]);
    for (const [path] of dshRuntimeMounts(input, options.runtime.dsh))
        out.add(path);
    for (const [, target] of options.runtime.dsh.nativeShims ?? [])
        out.add(target);
    for (const overlay of input.overlays)
        out.add(dirname(realpathSync(overlay)));
    return [...out];
}
...
export function taskContainerArgs(input, options) {
    ...
    if (options.runtime !== undefined) {
        ...
        const mounts = new Map(dshRuntimeMounts(input, options.runtime.dsh));
        ...
        for (const [source, target] of options.runtime.dsh.nativeShims ?? [])
    ...
    for (const k of ['DEEPSEEK_API_KEY'])
        if (input.env[k] !== undefined)
            args.push('-e', k);
    for (const k of Object.keys(input.arm.env ?? {}))
        args.push('-e', k);
...
export function taskContainerEnv(input) {
    const out = {};
    for (const k of ['DEEPSEEK_API_KEY'])
        if (input.env[k] !== undefined)
            out[k] = input.env[k];
...
export function taskRuntimeExecArgs(containerId, input, options) {
    if (options.runtime === undefined)
        throw new Error(`${options.image}: opened for grading only, with no runtime inside; a run needs the dsh checkout and Node`);
...
class ContainerEnvironment {
    ...
    mounts;
    log;
    constructor(id, workdir, image, platform, mounts, log) {
        this.id = id;
