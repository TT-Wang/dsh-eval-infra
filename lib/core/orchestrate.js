...
import { loadArmFile, applyRoute, DEFAULT_PROVIDER, DEFAULT_MODEL } from './arms.js';
...
import { projectPrices, benchPools } from './project.js';
import { buildReport, MIN_SCENARIOS, noiseFloorOf, readingAlpha, renderMarkdown, gradeOf, qualityReading } from './report.js';
import { fileSha, keyFingerprint, readManifest, readReceipt, receiptSignatureValid, reportDigest, sameKey, sealRun, signingKey, signReceipt, verifyRun, writeReceipt } from './manifest.js';
...
export async function launchRun(project, request, hooks = {}) {
    ...
    if (request.resume !== undefined) {
        ...
        if (conflicts.length > 0)
            throw new LaunchError(`--resume keeps the run's own conditions; drop these flags or start a new run — ${conflicts.join('; ')}`, 'usage');
        ...
        const route = { ...(request.model !== undefined ? { model: request.model } : {}), ...(request.effort !== undefined ? { effort: request.effort } : {}) };
        ...
        if (request.sequential && candidateSpecs.length > 1)
            throw new LaunchError(`sequential mode compares one candidate against the baseline (${candidateSpecs.length} given); run one candidate at a time or drop --sequential`, 'arms');
        if (request.sequential && (request.northStar ?? 'cost') !== 'cost')
            throw new LaunchError(`sequential mode decides on cost; the ${request.northStar} north star has no confidence sequence — drop --sequential or use --north-star cost`, 'arms');
        id = newRunId();
        plan = {
            id,
    ...
    if (hooks.onEvent !== undefined)
        deps.onEvent = hooks.onEvent;
    if (project.config.safety !== undefined)
        deps.safety = project.config.safety;
    ...
    if (request.probe) {
        ...
        // The probe gates the readings; it must never abort the run it is checking.
        let verdict;
        try {
            verdict = await probeRoute(project, { model, log: (l) => log(`  ${l}`) });
        }
        catch (e) {
            verdict = { model, distance: 0, p: 1, probes: 0, samplesPerSide: 0, verdict: 'not-completed', error: e instanceof Error ? e.message : String(e), comparedAt: new Date().toISOString(), usd: 0 };
        }
...
export async function probeRoute(project, options = {}) {
    const { batterySha, collectProbes, compareWithReference, referenceKey } = await import('./probe.js');
    ...
    const stored = existsSync(refFile) ? JSON.parse(readFileSync(refFile, 'utf8')) : null;
    const battery = batterySha();
    const stale = stored !== null && stored.batterySha !== undefined && stored.batterySha !== battery;
    if (stale)
        options.log?.('the stored reference was built from a different probe battery; re-enrolling instead of comparing');
    const reference = stale ? null : stored;
    const fresh = await collectProbes(chat, options.samples ?? 8, options.log);
    if (fresh.failures.length > 0 && fresh.samples.length < PROBE_COUNT * 2) {
        return { model, distance: 0, p: 1, probes: PROBE_COUNT, samplesPerSide: 0, verdict: 'not-completed', error: fresh.failures[0], comparedAt: new Date().toISOString(), usd: fresh.usd };
    }
    if (reference === null || options.enroll === true) {
        const ref = { schema: 'dsh-eval-probe/1', model, baseUrl, batterySha: battery, enrolledAt: new Date().toISOString(), samples: fresh.samples, usd: fresh.usd };
...
export async function runJudge(project, id, options = {}) {
    ...
    if (models === undefined || models.length === 0) {
        ...
        if (crossFamily !== undefined)
            models = [crossFamily.name ?? crossFamily.model];
        else if (options.allowSameFamily)
 ...
 */
export function receiptStatus(paths, base, report, trustedKeys = []) {
    let digestNote = '';
    ...
    if (base.manifestConsistent === false)
        return { status: 'INVALID', reason: 'the manifest\'s evidence sha does not follow from its own file list (the manifest was rewritten)' };
    ...
    if (!receiptSignatureValid(receipt))
        return { status: 'INVALID', reason: 'the receipt signature does not verify against the key it carries' };
    ...
    if (trusted === undefined)
        return { status: 'INCONCLUSIVE', reason: `the receipt is signed by a key this verifier does not trust (fingerprint ${keyFingerprint(receipt.publicKey)}); nothing is falsified, but a self-signed receipt proves only its own consistency — pass the author\'s published key with --key to verify it` };
    if (!receiptSignatureValid(receipt, trusted))
        return { status: 'INVALID', reason: 'the receipt signature does not verify against the trusted key' };
    ...
    if (onDisk !== null && receipt.evidenceSha !== onDisk)
        return { status: 'INVALID', reason: 'the receipt was issued for a different evidence set (evidence sha of the files on disk does not match)' };
    if (report !== null) {
        for (const claim of receipt.claims) {
            ...
            if (claim.reliability !== undefined && claim.reliability !== c.reliability.reading)
                return { status: 'INVALID', reason: `re-derived reliability reading for ${claim.arm} (${c.reliability.reading}) differs from the receipt (${claim.reliability})` };
            if (claim.northStar !== undefined && claim.northStar !== `${c.northStar.metric}:${c.northStar.reading}`)
                return { status: 'INVALID', reason: `re-derived north-star reading for ${claim.arm} (${c.northStar.metric}:${c.northStar.reading}) differs from the receipt (${claim.northStar})` };
        }
    }
    ...
    if (report !== null && receipt.reportSha !== undefined) {
        ...
        if (issuedBy !== undefined && issuedBy !== evalInfraVersion())
            digestNote = `report digest not compared: this verifier runs dsh-eval ${evalInfraVersion()}, the receipt was issued by ${issuedBy}; the receipted readings were compared instead`;
        else if (receipt.reportSha !== reportDigest(report))
            return { status: 'INCONCLUSIVE', reason: 'the evidence is intact and the receipted readings recompute, but the whole report derived now differs from the receipted digest in other fields: the code deriving reports has changed since the receipt was issued — rebuild the report and re-seal (dsh-eval report <run> then regrade/judge, or publish) to refresh the receipt' };
    }
...
export function writeRunContext(project, plan, paths, scenarios) {
    const context = {
...
export function readRunContext(paths) {
 ...
 */
export function reportOptionsFor(project, plan, paths) {
    ...
    let holdout = new Set();
    try {
        holdout = new Set(collectScenarios(project, { scenarios: plan.scenarios, includeHoldout: true }).scenarios.filter(s => s.meta.holdout).map(s => s.name));
    }
    catch { /* no library at hand: holdout unknown */ }
...
export function sequencesOf(paths) {
    ...
    try {
        ...
        return { sequences: { [seqFile.candidate]: { cost, pass: last.pass, reliability: last.reliability ?? null, scenarios: last.scenarios } } };
    }
    catch {
        return {};
    }
    ...
    for (const c of report.candidates) {
        ...
        if (j) {
            ...
            if (c.northStar.metric === 'quality') {
                ...
                if (c.gate === 'pass')
                    c.verdict = `${c.northStar.text}${c.improvements.length ? ` Improves correctness on ${c.improvements.join(', ')}.` : ''}`;
            }
            if (jj.abstention)
                report.notes.push(`${c.arm}: conformal abstention at α = ${jj.abstention.alpha} calibrated on ${jj.abstention.calibratedOn} human-labelled pair${jj.abstention.calibratedOn === 1 ? '' : 's'}: ${Number.isFinite(jj.abstention.tau) ? `threshold ${jj.abstention.tau.toFixed(2)}, ${jj.abstention.abstained} of ${jj.abstention.of} judgments withheld and not counted in the wins, losses or ties` : `no threshold meets the risk bound on that many labels (about ${Math.ceil(1 / jj.abstention.alpha)} are needed even with no judge errors), so nothing is withheld and the ≤ ${jj.abstention.alpha} error guarantee is not in force`}.`);
            else
 ...
 */
export function trustedReceiptKeys(project, extra = []) {
    const keys = [];
    const own = join(project.evalDir, 'receipt-key.json');
    if (existsSync(own)) {
        try {
            keys.push(JSON.parse(readFileSync(own, 'utf8')).publicKey);
        }
        catch { /* unreadable key file: nothing trusted from it */ }
    }
    for (const file of (process.env['DSH_EVAL_TRUSTED_KEYS'] ?? '').split(',').map(f => f.trim()).filter(Boolean)) {
        try {
            keys.push(readFileSync(file, 'utf8'));
        }
        catch { /* missing file: not trusted */ }
    }
    for (const k of extra)
        keys.push(k.includes('-----BEGIN') ? k : (() => { try {
...
export function verifyRunDir(project, dir, options = {}) {
    const paths = runPathsAt(dir);
    if (!existsSync(paths.plan))
        throw new LaunchError(`${dir} is not a run directory (no plan.json)`, 'usage');
    const id = readPlan(paths).id;
    const base = verifyRun(paths, () => ({ fresh: reportView(deriveReport(project, id, paths)), stored: existsSync(paths.report) ? reportView(JSON.parse(readFileSync(paths.report, 'utf8'))) : null }));
...
export function verifyRunIntegrity(project, id, options = {}) {
    const paths = runPaths(project.runsRoot, id);
    if (!existsSync(paths.plan))
        throw new LaunchError(`run ${id} not found`, 'usage');
    const base = verifyRun(paths, () => {
        const fresh = reportView(deriveReport(project, id));
        const stored = existsSync(paths.report) ? reportView(JSON.parse(readFileSync(paths.report, 'utf8'))) : null;
        return { fresh, stored };
    });
...
export function resealRun(project, id) {
    const paths = runPaths(project.runsRoot, id);
    if (readManifest(paths) === null)
        return; // not sealed yet (still running): the final seal will cover it
    const plan = readPlan(paths);
...
async function containerTaskSetup(project, platform, log) {
    ...
    const avail = await dockerAvailable();
    if (!avail.ok)
        throw new LaunchError(`container scenarios need Docker: ${avail.detail}`, 'env');
    const source = dshSourceRoot({ realpath: false });
    if (source === null)
        throw new LaunchError('container scenarios need a dsh source checkout (DSH_SOURCE or ~/.dsh/source/current)', 'env');
    ...
    if (platformIsEmulated(platform))
        log(`note: linux/${platform} images run under emulation on this ${process.arch} machine; every trial is slower than on a native host`);
...
export async function containerTaskRuntimeFactory(project, log) {
    const { openContainerTask } = await import('./environment.js');
    const setups = new Map();
    const setupFor = (platform) => {
        let p = setups.get(platform);
        if (p === undefined) {
            p = containerTaskSetup(project, platform, log);
            setups.set(platform, p);
        }
        return p;
    };
 ...
 */
export async function containerSelfcheckEnvironment(project, log) {
    ...
    return async (scenario) => {
        const platform = scenario.meta.platform ?? 'amd64';
        if (platformIsEmulated(platform) && !noted.has(platform)) {
            noted.add(platform);
            log(`note: linux/${platform} images run under emulation on this ${process.arch} machine; every check is slower than on a native host`);
        }
        const input = { arm: { name: 'selfcheck', profile: project.config.profile, provider: DEFAULT_PROVIDER, model: DEFAULT_MODEL, overlayPath: '', patchFilePaths: [] }, scenario, workdir: project.evalDir, evalHome: project.home, overlays: [], env: {} };
