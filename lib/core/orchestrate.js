...
import { applyAnnotations, listRuns, newRunId, readAnnotations, readLedgers, readPlan, runPaths, writeJsonAtomic, writeLedger } from './store.js';
...
export function deriveReport(project, id) {
    const paths = runPaths(project.runsRoot, id);
    if (!existsSync(paths.plan))
        throw new LaunchError(`run ${id} not found`, 'usage');
    const plan = readPlan(paths);
