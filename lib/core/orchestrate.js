...
import { fileSha, sealRun, verifyRun } from './manifest.js';
    ...
    for (const c of report.candidates) {
        ...
        if (j) {
            ...
            if (jj.lengthBalancedWinRate !== null && jj.lengthBalancedWinRate !== undefined)
                report.notes.push(`${c.arm}: length-balanced candidate win rate ${(jj.lengthBalancedWinRate * 100).toFixed(0)}% (average of the candidate-longer and candidate-shorter strata).`);
...
export async function rerunScenario(project, runId, scenario, options = {}) {
    ...
    const result = { scenario, candidate, newRunId: launched.id, reps, original, failedAgain, sameCall, verdict };
...
export function verifyRunDir(project, dir) {
    ...
    return verifyRun(paths, () => ({ fresh: pick(deriveReport(project, id, paths)), stored: existsSync(paths.report) ? pick(JSON.parse(readFileSync(paths.report, 'utf8'))) : null }));
}
