...
export const SCENARIO_FILES = ['meta.json', 'prompts.json', 'setup.py', 'verify.py', 'oracle.py', 'prompts.variants.json'];
const REQUIRED = ['meta.json', 'prompts.json', 'verify.py'];
export async function addScenario(project, name, files) {
    if (!/^[a-z0-9][a-z0-9_-]*$/i.test(name))
        throw new Error('the scenario name must be a short identifier: letters, digits, _ or -');
    const given = Object.keys(files);
    for (const f of REQUIRED)
        if (!given.includes(f))
            throw new Error(`${f} is required; a scenario needs at least meta.json, prompts.json and verify.py`);
    const unknown = given.filter(f => !SCENARIO_FILES.includes(f));
    if (unknown.length > 0)
        throw new Error(`unexpected file(s): ${unknown.join(', ')}. A scenario holds only ${SCENARIO_FILES.join(', ')}`);
    ...
    if (existsSync(dir))
        throw new Error(`${dir} already exists; delete it or choose another name`);
    ...
    try {
        ...
        if (scenario.name !== name)
            throw new Error(`meta.json names the scenario "${scenario.name}" but it is being added as "${name}"`);
...
export function scenarioTemplate(name) {
    return {
