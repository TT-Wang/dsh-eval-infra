...
export function loadProject(root = process.cwd()) {
    ...
    return {
        ...
        armsDir,
        benchRoot,
        // A configured root is honoured even when it is empty or absent: falling back to the shipped library
        // there would run thirty-five scenarios the user never wrote while they believed they ran their own.
        // The fallback is for the unconfigured default only, where `bench/scenarios` may simply not exist yet.
        scenarioRoot: config.scenarioRoot !== undefined ? ownScenarios : existsSync(ownScenarios) ? ownScenarios : bundled,
 ...
 */
export async function withPreviewArms(project, use) {
    mkdirSync(project.evalDir, { recursive: true });
...
export function benchPools(project) {
    if (!existsSync(project.benchRoot))
        return [];
