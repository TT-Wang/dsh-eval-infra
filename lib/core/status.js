...
import { readChecks, selfcheckStateOf } from './checks.js';
...
export function armNames(project) {
...
export async function projectStatus(project, options = {}) {
    ...
    let scenarios = [];
    try {
        scenarios = collectScenarios(project, {}).scenarios;
    }
    catch {
        scenarios = [];
    }
    ...
    const byState = { ok: [], failing: [], stale: [], never: [] };
    ...
    for (const s of scenarios)
        byState[selfcheckStateOf(checks.selfcheck[s.name], s.dir)].push(s.name);
    ...
    const recent = all.slice(0, options.runs ?? 5).map((r) => {
        ...
        if (existsSync(reportFile)) {
            try {
                gate = readJson(reportFile).candidates?.[0]?.gate;
            }
    ...
    const scenarioShortfall = scenarios.length < MIN_SCENARIOS ? [{ code: 'scenarios.below_minimum', have: scenarios.length, need: MIN_SCENARIOS }] : [];
    readings['reliability'] = availability(scenarioShortfall);
    const floorMissing = floorState === 'ok' ? []
    ...
    readings['northStar.efficiency'] = availability([
        ...scenarioShortfall,
        ...floorMissing,
        ...(floorState === 'ok' && floor?.steps === undefined ? [{ code: 'floor.no_step_band', runId: floor.runId }] : []),
    ]);
    const judgeScenarios = scenarios.filter(s => s.meta.judge !== undefined).length;
    readings['northStar.quality'] = availability([
        ...(judgeScenarios === 0 ? [{ code: 'scenario.no_judge_rubric' }] : []),
        ...((project.config.judges ?? []).length === 0 ? [{ code: 'judge.unconfigured' }] : []),
    ]);
...
function nextStep(phases, blocked, baseline, selfcheck, needsDocker) {
    const phase = (code) => phases.find(p => p.code === code);
    ...
    if (live.state === 'running')
        return { reason: { code: 'run.in_progress', id: live['id'] }, tool: 'run.status', args: { id: live['id'] } };
    if (blocked === 'profile')
        return { reason: { code: 'profile.missing', profile: phase('profile')['name'] }, tool: 'init' };
    if (blocked === 'arms') {
        const state = phase('arms').state;
        return { reason: { code: state === 'no_baseline' ? 'arms.no_baseline' : 'arms.no_candidate' }, tool: 'arms.write' };
    }
    if (blocked === 'scenarios')
        return { reason: { code: 'scenarios.none' }, tool: 'scenarios.add' };
    ...
    const run = phase('run');
    if (run.state === 'none')
        return { reason: { code: 'run.none' }, tool: 'run.start', args: { baseline, repeats: 3 } };
    return { reason: { code: 'run.readable' }, tool: 'report.read' };
}
/** The north stars a caller may register, for a tool listing them. */
export const NORTH_STAR_READINGS = ['northStar.cost', 'northStar.efficiency', 'northStar.quality'];
