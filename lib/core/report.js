...
export function buildReport(plan, ledgers, options = {}) {
    ...
    const candidates = plan.candidates.map((cand) => {
        ...
        if (metric === 'cost') {
            ...
            if (withSteps.length === 0) {
                blockers.push({ code: 'steps.no_comparable_pairs' });
                reading = 'none';
                text = 'No scenario where both arms passed; nothing to compare on steps.';
            }
            else if (withSteps.length < minScenarios) {
                blockers.push({ code: 'scenarios.below_minimum', have: withSteps.length, need: minScenarios });
                reading = 'inconclusive';
                ...
                if (noiseFloor === null) {
                    ...
                    blockers.push({ code: 'floor.no_step_band', runId: noiseFloor.runId });
                    reading = 'inconclusive';
                    ...
                    blockers.push({ code: 'reading.inside_noise_band', interval: [stepsCI.lo, stepsCI.hi], band: [noiseFloor.steps.lo, noiseFloor.steps.hi], floorRun: noiseFloor.runId });
                    reading = 'inconclusive';
                ...
                blockers.push({ code: 'reading.interval_covers_zero', interval: [stepsCI.lo, stepsCI.hi], sesoiPct: sesoi });
                reading = 'inconclusive';
        ...
        if (gate !== 'pass')
            blockers.unshift({ code: `gate.${gate}`, ...(gate === 'regressions' ? { scenarios: regressions } : gate === 'suspect' ? { scenarios: suspected } : gate === 'unsafe' ? { scenarios: unsafe } : { scenarios: incomplete }) });
    ...
    if (readState.length > 0)
        notes.push(`READ THE EVALUATION'S OWN FILES: ${readState.length} trial(s) opened files that belong to the evaluation rather than the task — ${[...new Set(readState.flatMap(l => l.evalFileReads.map(r => r.split(' — ')[0])))].join(', ')} (first: ${readState[0].evalFileReads[0].slice(0, 140)}). The scenario directory holds every turn's prompt and the verifier, and the run directory holds the evidence; a verdict from a trial that read either may reflect what it found there rather than what it did. Read those traces before crediting anything to the component, and run under --sandbox docker, where neither is mounted.`);
