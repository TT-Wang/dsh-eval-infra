 ...
 */
import { icc, mcnemar, mean, median, resolution, sequenceSimilarity, signTest, smallSampleCI, wilson, tCritical, stddev } from './stats.js';
...
export function buildReport(plan, ledgers, options = {}) {
    ...
    const candidates = plan.candidates.map((cand) => {
        ...
        const gate = regressions.length > 0 ? 'regressions' : ran.length === 0 || incomplete.length === ran.length ? 'incomplete' : 'pass';
    ...
    if (plan.sandbox === 'docker')
        notes.push('Trials ran inside Docker containers: the container is the confinement boundary (workspace and eval home mounted read-write, the dsh checkout and plugins read-only); dsh\'s in-process sandbox and permission presets were off inside the container.');
