 ...
 */
import { icc, mcnemar, mean, median, passPow, resolution, sequenceSimilarity, signTest, smallSampleCI, wilson, tCritical, stddev } from './stats.js';
...
export function buildReport(plan, ledgers, options = {}) {
    ...
    const candidates = plan.candidates.map((cand) => {
        ...
        const gate = unsafe.length > 0 ? 'unsafe' : regressions.length > 0 ? 'regressions' : ran.length === 0 || incomplete.length === ran.length ? 'incomplete' : 'pass';
        ...
        if (comparable.length > 0) {
            if (comparable.length < minScenarios)
                ...
                if (noiseFloor !== null && noiseFloor.scenarios >= minScenarios && Math.min(Math.abs(costPctCI.lo), Math.abs(costPctCI.hi)) < noiseFloor.meanAbsPct) {
                    costReading = 'inconclusive';
                    insideNoise = true;
                }
            }
        ...
        if (unreconciled.length > 0 && costReading !== 'none' && costReading !== 'inconclusive') {
            costReading = 'inconclusive';
        ...
        if (servedMismatch.length > 0 && costReading !== 'none') {
            costReading = 'inconclusive';
            servedBlocked = true;
        }
        ...
        if (gate === 'unsafe')
            ...
            verdict = `REGRESSION on ${regressions.length} scenario${regressions.length === 1 ? '' : 's'} (${regressions.join(', ')}); ${metric} is not compared until this is fixed.`;
            ...
            verdict = `Cost interval (${ciText}) reaches into the A/A noise band of ±${noiseFloor.meanAbsPct.toFixed(1)}% measured on this baseline; not read as a real difference.${gains}`;
    ...
    if (plan.candidates.length > 1)
        notes.push(`${plan.candidates.length} candidates share one baseline and each carries two planned claims (cost direction, pass-rate direction): intervals are read at α = ${alpha.toFixed(4)} (Bonferroni over ${2 * plan.candidates.length} claims) so the family-wise error rate stays at 5%.`);
    if (options.sequences)
        notes.push('Sequential mode: the cost interval is the final hedged betting confidence sequence on the paired cost ratio winsorized at 2× (non-asymptotic, valid at every look and under early stopping), which is wider than a fixed-sample interval would be on the same data.');
    ...
    for (const c of candidates) {
        ...
        if (c.noiseFloor !== null)
            notes.push(`${c.arm}: the A/A ${(c.noiseFloor?.kind ?? 'rerun') === 'perturbation' ? 'perturbation-floor ' : ''}run ${c.noiseFloor.runId} on this baseline showed |Δ%| averaging ${c.noiseFloor.meanAbsPct.toFixed(1)}% (interval ${fmtPct(c.noiseFloor.lo)} to ${fmtPct(c.noiseFloor.hi)}) with no real change; treat differences inside that band as noise.`);
        if (c.cuped !== null)
...
export function renderMarkdown(report) {
    ...
    for (const c of report.candidates) {
        ...
        lines.push('|---|---|---|---|---|---|---|---|---|---|');
        const order = { unsafe: 0, regression: 1, improvement: 2, 'both-fail': 3, incomplete: 4, same: 5, unrun: 6 };
