...
export function loadScenario(dir) {
    ...
    if (container) {
        if (typeof meta.image !== 'string' || meta.image === '')
            throw new ScenarioError(`${abs}: a container scenario needs meta.image`);
        ...
        if (!existsSync(join(abs, 'tests', 'test.sh')) && !existsSync(join(abs, 'verify.py')))
            throw new ScenarioError(`${abs}: missing tests/test.sh or verify.py (a container scenario is graded by the benchmark's own tests)`);
    }
    else if (!existsSync(join(abs, 'verify.py')))
 ...
 */
export function scenarioProcessEnv(base = process.env) {
    ...
    for (const [k, v] of Object.entries(base)) {
        if (v === undefined || !keep.test(k) || secret.test(k))
            continue;
        out[k] = v;
    }
    return out;
