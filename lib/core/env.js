...
export async function dshVersion(bin = process.env['DSH_BIN'] ?? 'dsh') {
    try {
 ...
 */
export function dshSourceRoot(options = {}) {
    const candidates = [process.env['DSH_SOURCE'], join(userDshHome(), 'source', 'current'), join(homedir(), '.dsh', 'source', 'current')].filter((c) => c !== undefined);
...
export async function dshSourceRevision(root) {
    if (root === null)
        return null;
    try {
...
export function evalInfraVersion() {
    try {
