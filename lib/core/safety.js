 ...
 */
export function outOfScopeWrites(entries, scope, ignores = DEFAULT_WRITE_IGNORES) {
    if (scope.includes('*'))
        return [];
...
export function evaluateSafety(input) {
    const out = [];
    if (input.diff !== null) {
        for (const e of outOfScopeWrites(parseDockerDiff(input.diff), input.scope, input.ignores))
