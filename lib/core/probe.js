...
export function probeDistance(a, b) {
    ...
    for (const p of probes) {
        ...
        let tv = 0;
        for (const k of keys)
            tv += Math.abs(xs.filter(v => v === k).length / xs.length - ys.filter(v => v === k).length / ys.length);
        total += tv / 2;
        counted += 1;
    }
...
export function probePermutationTest(a, b, B = 1000, seed = 11) {
    ...
    for (let i = 0; i < B; i += 1) {
        ...
        if (probeDistance(pa, pb) >= observed - 1e-12)
            atLeast += 1;
    }
...
export async function collectProbes(chat, samples = 8, log, concurrency = 8) {
    const jobs = [];
    ...
    const worker = async () => {
        for (;;) {
            ...
            let answer = '';
            try {
                answer = String(JSON.parse(text).answer ?? '');
            }
            catch {
                answer = text;
            }
    ...
    if (failures.length > 0)
        log?.(`${failures.length} probe call(s) failed after retries: ${[...new Set(failures)].slice(0, 2).join('; ')}`);
...
export function batterySha(probes = PROBES) {
    return createHash('sha256').update(probes.join('\n')).digest('hex');
}
export function referenceKey(model, baseUrl) {
...
export function compareWithReference(fresh, reference, model, usd, alpha = 0.01) {
    ...
    if (reference === null || (reference.batterySha !== undefined && reference.batterySha !== batterySha()))
        return { model, distance: 0, p: 1, probes: PROBES.length, samplesPerSide: 0, verdict: 'no-reference', comparedAt, usd };
