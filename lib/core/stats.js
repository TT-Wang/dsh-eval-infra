...
export function mcnemar(b, c, ropeHalfWidth = 0.1) {
    ...
    if (n === 0)
        return { b, c, exactP: 1, midP: 1, pWin: 0.5, inRope: 2 * ropeHalfWidth };
 ...
 */
export function passPow(passes, n, j) {
    if (j <= 0)
        return 1;
    if (n <= 0 || j > n)
        return 0;
    const choose = (a, b) => { if (b < 0 || b > a)
        return 0; let r = 1; for (let i = 1; i <= b; i += 1)
        r = r * (a - b + i) / i; return r; };
    return choose(passes, j) / choose(n, j);
}
export function resolution(values, alpha = 0.05) {
    const n = values.length;
    if (n < 2)
 ...
 */
export function bettingRejectsAt(xs, m, alpha = 0.05) {
    const c = 0.9;
    let plus = 1;
    let minus = 1;
    let sumX = 0;
...
export function bettingCS(xs, alpha = 0.05) {
    ...
    if (t === 0)
        return { lo: 0, hi: 1, t };
    const grid = 400;
    const survives = (m) => bettingRejectsAt(xs, m, alpha) === null;
    const scan = (from, to, points) => {
        let lo = 1;
        let hi = 0;
    ...
    if (lo > hi) {
        ...
        if (fine.lo <= fine.hi)
            return { lo: fine.lo, hi: fine.hi, t };
