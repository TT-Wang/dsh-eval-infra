...
import { hostVerifierEnvWithTimeout, INFRA_PREFIX, solveInEnvironment, verifyInEnvironment } from './environment.js';
...
async function selfcheckContainer(scenario, options) {
    ...
    if (options.taskEnvironment === undefined) {
        ...
        result.findings = [{ code: 'scenario.threw', phase: 'environment', message: result.error }];
        return result;
    }
    ...
    const grade = async (e) => {
        if (!hostSide)
            return verifyInEnvironment(e, join(scenario.dir, 'tests'), timeoutMs);
        ...
        if (!v.ok && v.detail.startsWith(INFRA_PREFIX))
...
export async function selfcheckScenario(scenario, workRoot = tmpdir(), options = {}) {
    ...
    try {
        ...
        if (scenario.hasOracle) {
            ...
            if (!solved.ok)
                findings.push({ code: 'oracle.rejected', detail: solved.detail.slice(0, 400) });
