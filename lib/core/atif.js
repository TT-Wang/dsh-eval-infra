function parseArgs(raw) {
    try {
        ...
        return v !== null && typeof v === 'object' && !Array.isArray(v) ? v : { raw };
    }
    catch {
        return { raw };
    }
}
export function toAtif(ledger, trace, prompts, version) {
    const steps = [];
    let id = 0;
    let lastTurn = 0;
    for (const row of trace) {
        if (row.turn !== lastTurn) {
            ...
            if (prompt !== undefined)
                steps.push({ step_id: ++id, timestamp: new Date(row.time).toISOString(), source: 'user', message: prompt });
        }
        const step = {
            step_id: ++id,
        ...
        if (row.reasoning !== '')
            step.reasoning_content = row.reasoning;
        if (row.calls.length > 0)
            step.tool_calls = row.calls.map((c, i) => ({ tool_call_id: row.observations[i]?.callId || `t${row.turn}-s${row.step}-${i + 1}`, function_name: c.name, arguments: parseArgs(c.arguments) }));
        if (row.observations.length > 0)
            step.observation = { results: row.observations.map((o, i) => ({ source_call_id: o.callId || `t${row.turn}-s${row.step}-${i + 1}`, content: o.text })) };
        if (row.usage !== null)
            step.metrics = { prompt_tokens: row.usage.hit + row.usage.miss, completion_tokens: row.usage.output, cached_tokens: row.usage.hit, cost_usd: row.usd };
        steps.push(step);
    }
    return {
