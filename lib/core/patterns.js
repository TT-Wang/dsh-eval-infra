...
export function failureSignature(reason) {
    return reason
        .toLowerCase()
 ...
 */
export function discoverPatterns(ledgers, minCount = 3) {
    const out = [];
    const add = (kind, signature, rows, example, denominator) => {
        ...
        for (const l of ledgers)
            armTotals.set(l.arm, (armTotals.get(l.arm) ?? 0) + 1);
    ...
    for (const [sig, rows] of bySig)
        add('failure', sig, rows, rows[0].error ?? rows[0].verdict?.detail ?? '', ledgers.length);
