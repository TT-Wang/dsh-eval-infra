...
function parseWinner(text) {
    try {
        ...
        return { winner: m?.[1] ?? 'tie', reason: text.slice(0, 300), confidence: 0.5 };
    }
}
/**
 ...
 */
export function conformalAbstentionThreshold(labelled, alpha) {
 ...
 */
export function bidirectionalPreferenceEntropy(votes, firstShown) {
    ...
    if (n === 0)
        return 1;
    let h = 0;
    for (const c of [counts.candidate, counts.baseline, counts.tie]) {
        const p = c / n;
        if (p > 0)
            h -= p * Math.log(p);
    }
...
export function abstentionScore(votes, firstShown = 'baseline') {
    if (votes.length === 0)
        return 0;
 ...
 */
export function effectiveJudges(votesPerItem, truth) {
    const k = votesPerItem[0]?.length ?? 0;
    if (k < 2 || votesPerItem.length < 3)
        return null;
    const basis = truth && truth.filter(t => t !== null && t !== undefined).length >= 3 ? 'error' : 'vote';
    const series = [];
    for (let j = 0; j < k; j += 1) {
        const col = [];
        for (let i = 0; i < votesPerItem.length; i += 1) {
            ...
            if (basis === 'error') {
                const t = truth[i];
                col.push(t === null || t === undefined ? 0 : v === t ? 0 : 1);
            }
            else
                col.push(v === 'candidate' ? 1 : v === 'baseline' ? -1 : 0);
        }
        series.push(col);
    }
    const corr = (a, b) => {
    ...
    for (let i = 0; i < k; i += 1)
        for (let j = i + 1; j < k; j += 1)
            rs.push(corr(series[i], series[j]));
 ...
 */
export function equalLengthWinRate(pairs) {
...
export async function gradeAnchors(anchors, judges, log) {
    const answers = {};
    let usd = 0;
    let agree = 0;
    let stableN = 0;
    ...
    for (const a of anchors) {
        ...
        if (pass === a.humanPass)
            agree += 1;
        if (a.previousJudgePass !== undefined) {
            stableN += 1;
            if (a.previousJudgePass === pass)
                stable += 1;
        }
    ...
    const summary = n ? { n, humanAgreement: agree / n, stability, comparedWithPrevious: stableN, attribution: stability !== null && stability < 0.8 ? 'judge' : 'none' } : null;
...
export async function judgeRun(input) {
    ...
    const interJudgeKappa = input.judges.length >= 2 ? kappa(judgments.map(j => [j.votes[0].preference, j.votes[1].preference])) : null;
    ...
    const share = (xs) => xs.filter(j => j.preference === 'candidate').length / xs.length;
