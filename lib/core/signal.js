...
export function scenarioSignal(arms) {
    const meanOf = (xs) => xs.reduce((a, b) => a + b, 0) / xs.length;
    const varOf = (xs) => { const m = meanOf(xs); return xs.reduce((a, b) => a + (b - m) ** 2, 0) / (xs.length - 1); };
    ...
    if (usable.length < 2)
        return { snr: null, withinCv: null, passSpread, trials };
...
export function archiveSignal(runsRoot, exceptRunId) {
    ...
    for (const r of listRuns(runsRoot)) {
        ...
        let ledgers;
        try {
            ledgers = readLedgers(runPaths(runsRoot, r.id));
        }
        catch {
            continue;
        }
    ...
    const out = new Map();
    for (const [name, arms] of byScenario)
        out.set(name, scenarioSignal([...arms.entries()].map(([arm, c]) => ({ arm, ...c }))));
    return out;
}
...
export function archiveSignalOrder(runsRoot, exceptRunId) {
