...
import { NORTH_STARS } from './core/types.js';
...
function parseNorthStar(value) {
    if (NORTH_STARS.includes(value))
        return value;
...
async function cmdScenarios(project, args) {
...
/**
 * Where the project is and what to do next, for the agent driving the evaluation.
 * `--json` is the real interface (docs/agent-interface.md); the plain form is a
 * legible rendering of the same object for a person at a terminal.
 */
async function cmdStatus(project, args) {
    ...
    const brief = (v) => typeof v === 'number' ? (Number.isInteger(v) ? String(v) : v.toFixed(1))
        : Array.isArray(v) ? (v.length > 2 ? `${v.length}` : v.map(brief).join(','))
            : typeof v === 'object' && v !== null ? Object.entries(v).map(([k, x]) => `${k}:${brief(x)}`).join('/')
                : String(v);
    out(`project ${tilde(st.project.root)} · profile ${st.project.profile}`);
    for (const p of st.phases) {
        const detail = Object.entries(p).filter(([k]) => k !== 'code' && k !== 'state').map(([k, v]) => `${k}=${brief(v)}`).join(' ');
        out(`  ${p.state === 'ok' ? 'ok ' : p.state === 'unchecked' ? '?  ' : '-> '} ${p.code.padEnd(13)} ${p.state.padEnd(10)} ${detail}`);
    }
    const notYet = Object.entries(st.readings).filter(([, r]) => !r.attainable);
    if (notYet.length > 0) {
        out('  not attainable yet:');
        for (const [name, r] of notYet)
            out(`    ${name.padEnd(22)} ${(r.missing ?? []).map(m => m.code).join(', ')}`);
    }
    const nextArgs = st.next.args === undefined ? '' : ' ' + Object.entries(st.next.args).map(([k, v]) => `${k}=${brief(v)}`).join(' ');
    out(`  next: ${st.next.tool}${nextArgs}  (${st.next.reason.code})`);
    return 0;
}
...
async function cmdRun(project, args) {
    ...
    if (progress.status !== 'done')
        return 2;
    if (report.candidates.some(c => c.gate === 'regressions' || c.gate === 'unsafe'))
        return 1;
...
async function cmdReport(project, args) {
    const id = args.positional[0];
    if (id === undefined) {
        err('usage: dsh-eval report <runId> [--json] [--claims] [--rebuild-ledgers]');
        return 3;
    }
...
async function cmdPublish(project, args) {
    ...
    const { sealRun, signingKey, readReceipt, keyFingerprint } = await import('./core/manifest.js');
    ...
    const bundleReceipt = readReceipt(runPathsAt(outDir));
    const signer = bundleReceipt ? keyFingerprint(bundleReceipt.publicKey) : 'none';
    writeFileSync(join(outDir, 'VERIFY.md'), [
        ...
        `The receipt is signed by the author's Ed25519 key, fingerprint \`${signer}\`. A verifier trusts only the keys it is given: compare that fingerprint with the one the author published, save the author's public key (the \`publicKey\` field of receipt.json, once you trust it) as a .pem file, and pass it with \`--key\`; without a trusted key the verdict is INCONCLUSIVE, because a receipt checked only against the key it carries proves its own consistency and nothing else.`, '',
        'The command recomputes every hash, lists missing or changed files, re-derives the report from the ledgers and the run\'s pinned archive context, compares the whole derived report with the receipted digest, and checks the signed receipt against the trusted key. It answers PASS (signed claims recompute from intact evidence, exit 0), INVALID (evidence, signature or derivation broken, exit 1) or INCONCLUSIVE (nothing falsified, but the run has no contract or its evidence is incomplete, exit 2).', '',
...
function cmdVerify(project, args) {
    const id = args.positional[0];
    if (id === undefined) {
        err('usage: dsh-eval verify <runId | run directory> [--json] [--key <public-key.pem>]');
        return 3;
    }
    const keys = [].concat(...[args.flags['key']].flat().filter((k) => typeof k === 'string').map(k => k.split(',')));
    const v = existsSync(join(id, 'plan.json')) ? verifyRunDir(project, id, { keys }) : verifyRunIntegrity(project, id, { keys });
    if (args.flags['json'] === true) {
...
function cmdProgress(project, args) {
    const id = args.positional[0];
    if (id === undefined) {
        err('usage: dsh-eval progress <runId> [--json]');
        return 3;
    }
    const paths = runPaths(project.runsRoot, id);
    if (!existsSync(paths.plan)) {
    ...
    if (!existsSync(paths.progress)) {
        err(`run ${id} has no progress file`);
        return 3;
    }
    ...
    for (const a of progress.active)
        out(`  running ${a.scenario}/${a.arm}#${a.rep} turn ${a.turn}/${a.turns}`);
    if ('abandoned' in body)
        out(`  no progress for ${(idleMs / 60000).toFixed(0)} min: the process is gone`);
    return 0;
}
function cmdRuns(project, args) {
    const runs = listRuns(project.runsRoot);
    if (args.flags['json'] === true) {
        out(JSON.stringify({
            ...
            runs: runs.map((r) => {
                ...
                if (existsSync(paths.report)) {
                    try {
                        gate = readJson(paths.report).candidates[0]?.gate;
                    }
...
async function cmdBench(project, args) {
    ...
    if (sub === undefined || sub === 'help') {
        out('usage: dsh-eval bench list <dataset> [--refresh] | get <dataset> <task>... [--no-pull] | rm <dataset> <task>...\ndatasets: ' + Object.values(ADAPTERS).map(a => `${a.id} (${a.title} ${a.version}, ${a.license})`).join(', '));
        return 3;
    }
    if (dataset === undefined) {
        err('name a dataset: ' + Object.keys(ADAPTERS).join(', '));
        return 3;
    }
    ...
    if (sub === 'list') {
        ...
        for (const t of index.tasks)
            out(`${present.has(t.id) ? '✓' : ' '} ${t.id.padEnd(36)} ${(t.category ?? '').padEnd(22)} ${(t.difficulty ?? '').padEnd(8)} ${t.image}`);
        out(`\n✓ = in this project (${adapter.poolDir(project).replace(project.root + '/', '')}). Get one: dsh-eval bench get ${adapter.id} <task>`);
        return 0;
    }
    if (sub === 'get') {
        if (rest.length === 0) {
            err('name at least one task');
            return 3;
        }
    ...
    if (sub === 'rm') {
        for (const id of rest)
            out(`${id}: ${adapter.remove(project, id) ? 'removed' : 'not present'}`);
        return 0;
    }
    err(`unknown bench command ${sub}`);
    return 3;
}
...
SET UP
  ...
  mcp                                 serve the agent interface over stdio as an MCP server (docs/agent-interface.md)
...
export async function main(argv = process.argv.slice(2)) {
    ...
    switch (args.command) {
        ...
        case 'mcp': {
            // The stdio transport owns stdout: nothing but protocol frames may go there.
            const { serveMcp } = await import('./mcp/index.js');
            await serveMcp({ projectRoot: project.root });
            return 0;
        }
        case 'status': return await cmdStatus(project, args);
