...
const strs = (v) => (Array.isArray(v) ? v.filter((x) => typeof x === 'string') : undefined);
export const TOOLS = [
    {
        name: 'status',
    ...
    },
    {
        ...
        async run(project, args) {
            ...
            if (scenarios.length === 0)
                return { error: { code: 'scenarios.none_matched', asked: names ?? [] } };
    ...
    },
    {
        ...
        async run(project, args) {
            ...
            if (baseName === undefined || candNames.length === 0)
                return { error: { code: 'arms.missing', need: ['baseline', 'candidates'] } };
    ...
    },
    {
        ...
        async run(project, args) {
            ...
            if (baseline === undefined)
                return { error: { code: 'arms.missing', need: ['baseline'] } };
            const aa = args['aa'] === true;
            const candidates = strs(args['candidates']) ?? [];
            if (!aa && candidates.length === 0)
                return { error: { code: 'arms.no_candidate', hint: 'pass candidates, or aa: true to measure the floor' } };
            const launched = await launchRun(project, {
                ...
                ...(str(args['northStar']) !== undefined ? { northStar: str(args['northStar']) } : {}),
    ...
    },
    {
        ...
        async run(project, args) {
            ...
            if (id === undefined)
                return { error: { code: 'run.missing_id' } };
            const paths = runPaths(project.runsRoot, id);
            if (!existsSync(paths.progress))
                return { error: { code: 'run.not_found', runId: id } };
...
export function serveMcp(options = {}) {
    ...
    const call = async (name, args) => {
        ...
        if (tool === undefined)
            return { text: JSON.stringify({ error: { code: 'tool.unknown', name, known: TOOLS.map(t => t.name) } }), isError: true };
        try {
    ...
    return new Promise((resolve) => {
        ...
        rl.on('line', (line) => {
            const handled = (async () => {
                ...
                switch (frame.method) {
                    case 'initialize': {
                        const asked = frame.params?.['protocolVersion'] ?? '';
                        reply({
                    ...
                    default:
                        if (id !== undefined)
                            send({ jsonrpc: '2.0', id, error: { code: -32601, message: `method not found: ${frame.method}` } });
                }
            })();
