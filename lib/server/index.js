...
export function scenarioSignal(arms) {
    ...
    if (usable.length < 2)
        return { snr: null, withinCv: null, passSpread: arms.length >= 2 ? Math.max(...arms.map(a => a.passes.reduce((x, y) => x + y, 0) / Math.max(1, a.passes.length))) - Math.min(...arms.map(a => a.passes.reduce((x, y) => x + y, 0) / Math.max(1, a.passes.length))) : null, trials };
