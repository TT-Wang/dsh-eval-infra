...
import { loadArmFile, parseArm, applyRoute, DEFAULT_MODEL } from '../core/arms.js';
import { load as parseYaml } from 'js-yaml';
import { toAtif } from '../core/atif.js';
import { evalInfraVersion, tilde } from '../core/env.js';
import { collectScenarios, launchRun, LaunchError, rebuildReport, resealRun, resolveArmPath, verifyRunIntegrity } from '../core/orchestrate.js';
import { scenarioSignal } from '../core/signal.js';
import { describeDiff, evalProfileManifest, prepareArms } from '../core/plan.js';
import { loadProject, withPreviewArms } from '../core/project.js';
...
async function readBody(req) {
    ...
    if (type !== 'application/json')
        throw new RequestError(415, `request bodies must be application/json (got ${type || 'no content type'})`);
 ...
 */
export function crossSiteRequest(req) {
    ...
    if (typeof origin === 'string' && origin !== '' && origin !== 'null') {
        ...
        if (host.toLowerCase() !== own.toLowerCase())
            return `cross-site request refused (Origin ${origin} is not this server ${own || '(no Host header)'})`;
    }
    else if (typeof origin === 'string')
        return 'cross-site request refused (opaque Origin)';
    return null;
}
...
export class EvalApp {
    ...
    async start(request) {
        ...
        const launched = await launchRun(this.project, request, {
            ...
            onEvent: (trial, e) => { const a = activityOf(trial, e); if (a !== null)
                emit('activity', a); },
        });
    ...
    async handle(req, res, prefix = '') {
        ...
        try {
            ...
            if (status === 500)
                process.stderr.write(`dsh-eval ui: ${req.method ?? 'GET'} ${path} failed\n${error instanceof Error ? (error.stack ?? error.message) : String(error)}\n`);
    ...
    async api(req, res, path, url) {
        ...
        if (method === 'GET' && path === '/arms') {
            ...
            const arms = files.map((f) => {
                ...
                try {
                    ...
                    return { file: f, path: tilde(p), spec, text: readFileSync(p, 'utf8') };
                }
                catch (e) {
                    return { file: f, path: tilde(p), error: e instanceof Error ? e.message : String(e), text: readFileSync(p, 'utf8') };
                }
            });
        ...
        if (method === 'GET' && path === '/bench') {
            ...
            for (const a of Object.values(ADAPTERS)) {
                ...
                let index = null;
                try {
                    index = await a.index(project, { refresh: url.searchParams.get('refresh') === '1' });
                }
                catch (e) {
        ...
        if (method === 'POST' && path === '/bench/get') {
            const body = await readBody(req);
            if (typeof body.dataset !== 'string' || typeof body.task !== 'string') {
                json(res, 400, { error: 'dataset and task are required' });
                return;
            }
            if (!validTaskId(body.task)) {
                json(res, 400, { error: 'task must be a single path segment: letters, digits, . _ -' });
                return;
            }
        ...
        if (method === 'GET' && path === '/rows') {
            ...
            if (armName === '') {
                json(res, 400, { error: 'no arm named' });
                return;
            }
        ...
        if (method === 'POST' && path === '/arms/diff') {
            const body = await readBody(req);
            if (typeof body.baseline !== 'string' || body.baseline === '') {
                json(res, 400, { error: 'no baseline arm named' });
                return;
            }
            if (body.candidates !== undefined && !Array.isArray(body.candidates)) {
                json(res, 400, { error: 'candidates must be a list of arm names' });
                return;
            }
