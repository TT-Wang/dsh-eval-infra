...
- **序贯模式**(`--sequential`):场景按固定种子随机排序,配对成本的渐近置信序列和通过率差的下注序列一旦作出判定就提前停止;报告读的是这条在提前停止下依然有效的序列,而不是固定样本的 bootstrap。
- **盲评判卷**(`dsh-eval judge <run>`):声明了评分标准和产出文件的场景,由看不到臂名和模型名的判卷模型按两种顺序比较基线与候选的产出;两种顺序不一致算平局;多个 `--model` 组成评审团,按绝对多数决;报告给出顺序不一致率与全票一致率;`--mode absolute` 逐次试验打分,并用运行里已有的人工标注做 PPI++ 校正。
- **容器隔离**(`--sandbox docker`):每次试验的 dsh 运行时在独立容器里跑,只挂载只读的 dsh 源码与插件、eval home 和试验工作区。
...
3. **区间**。各场景的 Δ% 以场景为簇做 bootstrap(每个簇带上它全部的重复对;B=2000,固定种子),并报告重复间的组内相关与设计效应。"更便宜/更贵"要求至少五个可比场景、区间不含零、且区间不落入已测得的 A/A 噪声带;"等价"要求五个场景且区间落在 ±10% 内;场景少于十个时用 t 区间而不是 bootstrap;其他都写"不确定",场景少时这是诚实的默认。备注里给出本次设计的最小可检出效应、观测效应的分辨率 q = n/N*,有 A/A 时给出噪声地板。配对通过/失败用 McNemar mid-p 检验不一致对,并给出"候选更优"的后验概率。
...
| `judge <id> [--model M] [--arm A] [--seed N]` | 对声明了 `meta.judge` 的场景做盲评配对判卷 |
| `runs` | 列运行 |
...
- 主机模式靠 dsh 自带沙箱;容器模式靠 Docker 挂载隔离(容器是边界,dsh 的进程内沙箱在容器里关闭)。
- 跨厂商家族的评审团需要你自己配置端点;默认评审团只有 DeepSeek 模型。
