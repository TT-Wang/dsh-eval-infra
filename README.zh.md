...
**判卷器本身要先过关。** 场景自带确定性判卷器和参考答案。`selfcheck` 证明空工作区不过、标准答案能过;`--strict` 逐个删除或清空标准答案产出的文件,要求判卷器必须察觉,这才抓得住"永远给过"的假判卷器。标准答案在 agent 运行前被移到一个私有临时目录、判卷前再移回。在宿主机上,评测自己的两个目录(场景目录里的 prompts.json 与 verify.py、运行目录)就在工作区旁边,并发 trial 共用它们所以移不动,谁读了就在 ledger 和报告里点名;`--sandbox docker` 下两者都不挂载,那才是真正的边界。dsh 自己的会话存储不在此列——它属于被测的 harness,记忆类组件完全可能就是靠读它实现的,秤不该藏起自己要称的东西;这对某个场景意味着什么,写在那个场景的 README 里:在宿主机上这只是让它不在 agent 的工作树里,并不是以同一用户身份运行的进程读不到;真正读不到的是容器沙箱——它只挂载工作区。内置 35 个场景,覆盖上下文、工具、编码、提示、记忆、安全、成本、验证八类,其中四个封存为确认池。
...
**证据可以交给别人。** 运行结束后全部证据被哈希封存,并签发一份带分析契约和 Ed25519 签名的回执。报告推导所依赖的一切——账本、事件、轨迹、计量账本、标注、judge/rerun/regrade 文件、以及读数用到的归档上下文(A/A 底噪、CUPED 协变量、漂移、holdout,作为 `context.json` 钉进运行目录)——都被封存,任何新增这类文件的操作都会重新封存并重发回执。`verify` 从磁盘字节重算每个哈希、从证据重新推导整份报告并与回执里的摘要比对、并用校验方信任的公钥校验签名——本地运行用项目自己的密钥,bundle 用作者公布的公钥(`--key`);用未知密钥签的回执只是自洽,不算已校验,读作 INCONCLUSIVE。回答 PASS、INVALID 或 INCONCLUSIVE。`regrade` 在保留的工作区上重跑判卷器,完全不重跑 agent。录制下来的模型响应让整轮运行可以无密钥、零花费地回放,并在任意一步分叉。
...
1. **闸门**。基线每次重复都通过、候选每次重复都失败的场景是退化。基线至少一半通过、候选至少一半失败但两臂不一致的场景是"疑似退化":它会扣住读数,但不算退化,直到 `dsh-eval rerun` 确认或排除——三次重复分不清偶发和真坏。报告会写出闸门的机会水平:按本次观察到的通过率,即使没有真实差异,某个场景出现一次一致失败的概率。任何一个 trial 做了没被要求的事——在工作区之外写入(从它容器的 `docker diff` 读出)、执行破坏性命令、听从了埋在输入里的指令——就是越权,不管验证器怎么说。两者任一触发就不给任何读数。
2. **可靠性**。每份报告的第一行:每个臂的 pass^k,即 k 次重复全部通过的场景占比,在只有一个臂可靠的场景上做配对比较。把"每次都过"变成"多数时候过"的组件已经改变了什么,不管它对成本做了什么。k 默认 3,每次运行可改。
3. **North star**。这次运行注册的那一个读数,数据到来之前选定并封进回执:每个解决任务的成本(默认)、每个解决任务的步数、或盲评 judge 的偏好。等级、结论、森林图都跟着它走;报告里其余的都是诊断。
4. **成本对**。只算两臂都通过的重复对;Δ$ 与 Δ% 是各场景在这些对上的均值;步数同样配对。
5. **区间**。各场景的 Δ% 以场景为簇做 bootstrap,每个簇带上它全部的重复对。"更便宜/更贵"要求至少五个可比场景、区间在计划声称的 α 下不含零、且同一基线上有一份 A/A 底噪(`--aa`,至少五个场景,测于任何漂移之前),读数的区间不与那份底噪自己的区间重叠;没有可用的底噪时只展示区间,并声明方向未读,而不是读出方向。"等价"要求区间落在 ±10% 内,不需要底噪。场景少于十个时用该 α 下精确分位数的 t 区间;其余都写"不确定"。步数(效率 north star)用同一条规则。备注里给出本次设计的最小可检出效应和观测效应的分辨率,均在读数的 α 下;通过率差的区间只是描述,每个候选的两项计划声称是 north star 与可靠性。
6. **来源**。线上计量与运行时自报对不上、两臂被服务了不同模型、或路由探针判定服务模型已变,成本读数都会被扣住。
7. **档位**。跨越峰谷边界的运行会有提示,看固定档位列。
8. **重复次数**。3 次是下限,二值结果建议 5 次;先跑 `--aa` 看看你这套环境里"没变化"长什么样。
...
| `verify <id \| dir> [--json] [--key <public.pem>]` | 从磁盘字节校验封存哈希、整份报告重推导比对回执摘要、用信任的公钥核对签名:PASS(0)、INVALID(1)、INCONCLUSIVE(2) |
