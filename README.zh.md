...
**判卷器本身要先过关。** 场景自带确定性判卷器和参考答案。`selfcheck` 证明空工作区不过、标准答案能过;`--strict` 逐个删除或清空标准答案产出的文件,要求判卷器必须察觉,这才抓得住"永远给过"的假判卷器。标准答案在 agent 运行前被移到一个私有临时目录、判卷前再移回;主动结束会话的场景,其会话记录(以及保存首条 prompt 的明文缓存)也会先移出 eval home,免得记忆题靠读 harness 自己的文件就能答对。在宿主机上,评测自己的三个目录(会话存储、场景目录里的 prompts.json 与 verify.py、运行目录)就在工作区旁边:能移的已经移走,移不动的会在 ledger 和报告里点名;`--sandbox docker` 下三者都不挂载,那才是真正的边界:在宿主机上这只是让它不在 agent 的工作树里,并不是以同一用户身份运行的进程读不到;真正读不到的是容器沙箱——它只挂载工作区。内置 35 个场景,覆盖上下文、工具、编码、提示、记忆、安全、成本、验证八类,其中四个封存为确认池。
