...
- On the host, two of the evaluation's own directories sit beside the workspace and answer questions the task is asking: the scenario directory (`prompts.json` carries every turn's text, `verify.py` the criteria) and the run directory (the ledgers, traces and recorded provider responses of both arms). Concurrent trials share both, so neither can be moved out of the way; a trial that opens either is recorded on its ledger and named in the report's notes — a flag, not a wall. Under `--sandbox docker` neither is mounted (the container gets the overlays' own directory, not the run directory), which is the actual boundary.
- dsh's own session store keeps every session's transcript under the eval home, which is mounted in both modes. A scenario that asks the agent to recall something said in an earlier session can therefore be answered by reading that transcript instead of remembering it. dsh-eval does not hide it: it belongs to the harness under test, and a memory component may legitimately be implemented by reading it. What that costs a given scenario is stated in that scenario's README (`m1_cross_session_recall`).
