...
- **Guard rails**: `selfcheck --strict` mutates every oracle output to catch verifiers that ignore it; `--max-usd` caps spend; dsh's workspace-write sandbox confines every trial's shell to its own workspace.
