...
- **Judge safeguards**: same-family judges are refused by default; conformal abstention calibrated on human labels, anchor-set re-grading for judge drift, length-balanced win rate, TPR/TNR, inter-judge κ.
...
| `rerun <id> <scenario> [--repeats 3]` | rerun one scenario's pair to validate a failure and its divergence point; result shown in the original report |
...
- A judge from the arms' model family is refused unless `--allow-same-family`; a second-family endpoint must be configured by you. Abstention needs human-labelled pairs to calibrate; anchors need annotated trials in the archive.
- Deliberately not done: adaptive scenario selection (it changes what the confidence sequence covers), simulated users (both arms must see identical inputs), predicted early termination (a predicted failure is not a measured one). Served-model checks read the provider's declared model plus the baseline's behavioural drift, not a per-trial behavioural fingerprint.
