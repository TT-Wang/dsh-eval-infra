...
- On the host, two of the evaluation's own directories sit beside the workspace and answer questions the task is asking: the scenario directory (`prompts.json` carries every turn's text, `verify.py` the criteria) and the run directory. Concurrent trials share both, so neither can be moved; a trial that opens either is recorded on its ledger and named in the report's notes — a flag, not a wall. Under `--sandbox docker` neither is mounted, which is the actual boundary.
