...
export async function launchRun(project: Project, request: RunRequest, hooks: LaunchHooks = {}): Promise<Launched> {
  ...
  if (request.probe) {
    ...
    try { verdict = await probeRoute(project, { model, log: (l) => log(`  ${l}`) }) }
    catch (e) { verdict = { model, distance: 0, p: 1, probes: 0, samplesPerSide: 0, verdict: 'not-completed', error: e instanceof Error ? e.message : String(e), comparedAt: new Date().toISOString(), usd: 0 } }
...
export async function probeRoute(project: Project, options: { model?: string; samples?: number; enroll?: boolean; chat?: import('./judge.js').ChatCall; log?: (line: string) => void } = {}): Promise<ProbeVerdict> {
  ...
  const stored: ProbeReference | null = existsSync(refFile) ? (JSON.parse(readFileSync(refFile, 'utf8')) as ProbeReference) : null
  const battery = batterySha()
  const stale = stored !== null && stored.batterySha !== undefined && stored.batterySha !== battery
  if (stale) options.log?.('the stored reference was built from a different probe battery; re-enrolling instead of comparing')
  ...
  if (reference === null || options.enroll === true) {
    const ref: ProbeReference = { schema: 'dsh-eval-probe/1', model, baseUrl, batterySha: battery, enrolledAt: new Date().toISOString(), samples: fresh.samples, usd: fresh.usd }
