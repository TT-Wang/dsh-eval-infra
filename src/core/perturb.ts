...
export interface VariantResult { variants: string[][]; usd: number; rejected: string[] }

export async function generateVariants(scenario: Scenario, n: number, chat: ChatCall, seed = 1): Promise<VariantResult> {
  ...
  for (let k = 0; k < n; k += 1) {
    ...
    let parsed: unknown
    try { parsed = JSON.parse(res.text) } catch { rejected.push(`variant ${k + 1}: not JSON`); continue }
    const prompts = (parsed as { prompts?: unknown }).prompts
    if (!Array.isArray(prompts) || prompts.length !== scenario.prompts.length || prompts.some(p => typeof p !== 'string' || p.trim().length === 0)) { rejected.push(`variant ${k + 1}: wrong shape`); continue }
    const list = (prompts as string[]).map(p => p.trim())
    if (list.every((p, i) => p === scenario.prompts[i])) { rejected.push(`variant ${k + 1}: identical to the original`); continue }
    variants.push(list)
  }
  return { variants, usd, rejected }
}

export function writeVariants(scenario: Scenario, variants: string[][]): string {
