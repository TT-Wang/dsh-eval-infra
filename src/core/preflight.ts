 ...
 */
export function explainRuntimeFailure(error: string): string {
  ...
  if (specific !== undefined) return specific.replace(/^(Error|TypeError):\s*/, '')
  const anyError = lines.find(l => /^(Error|TypeError|RangeError):/.test(l))
  if (anyError !== undefined) return anyError.replace(/^\w+Error:\s*/, '').replace(/^Error:\s*/, '')
  return lines[0] ?? error
}

/** Rows an arm intends to add: inserts from its own patches and from any patch file it applies. */
function intendedRows(project: Project, armName: string): Array<{ id: string; name?: string }> {
  const spec = loadArmFile(resolveArmPath(project, armName))
  const out: Array<{ id: string; name?: string }> = []
  for (const patch of spec.patches ?? []) {
    const p = patch as Record<string, unknown>
    if (!Array.isArray(p['insert'])) continue
    for (const entry of p['insert']) {
      const e = entry as Record<string, unknown>
      if (typeof e['id'] === 'string') out.push({ id: e['id'], ...(typeof e['name'] === 'string' ? { name: e['name'] } : {}) })
    }
  }
  return out
}
...
async function preflightIn(project: Project, armName: string, options: PreflightOptions, armsDir: string): Promise<PreflightResult> {
  ...
  if (scenario === undefined) throw new LaunchError(`smoke scenario ${scenarioName} not found in ${project.scenarioRoot}`, 'usage')
  if (options.driverFactory === undefined && resolveApiKey() === undefined) throw new LaunchError('DEEPSEEK_API_KEY not found (env, $DSH_HOME/.env or ~/.dsh/.env)', 'env')
  ...
  await executeRun(plan, [scenario], resolved, {
    ...
    log: (line: string) => log(line.split('\n')[0] ?? line),
  } as Parameters<typeof executeRun>[3])
