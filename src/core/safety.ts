 ...
 */
export function evaluationFileReads(events: EventLike[], paths: EvalPaths): string[] {
  ...
  for (const e of events) {
    ...
    if (text === '') continue
    // A path inside the workspace is the task's own file, whatever it is named.
    const outside = workdir === '' || !text.includes(workdir)
    const kind = (scenario !== '' && text.includes(scenario)) || (outside && /[/\\](prompts\.json|verify\.py|oracle\.py|prompts\.variants\.json)\b/.test(text)) ? "the scenario's own directory"
