 ...
 */
import { icc, mcnemar, mean, median, normalQuantile, passPow, resolution, sequenceSimilarity, signTest, smallSampleCI, wilson, tCritical, stddev, type BootstrapCI } from './stats.js'
...
export interface NoiseFloor {
  ...
  steps?: { lo: number; hi: number; scenarios: number }
}
...
export interface ReportOptions {
  ...
  sequences?: Record<string, { cost: { mean: number; lo: number; hi: number } | null; pass: { lo: number; hi: number } | null; reliability?: { lo: number; hi: number } | null; scenarios: number }>
}
...
export function gradeOf(gate: CandidateReport['gate'], improvements: number, ns: NorthStarReading['reading']): Grade {
  ...
  if (gate === 'incomplete' || gate === 'suspect') return 'inconclusive'
...
export function noiseFloorOf(plan: RunPlan, ledgers: RunLedger[], alpha = readingAlpha({ candidates: [{ name: 'one' }] })): NoiseFloor | null {
...
export function withinNoise(ci: { lo: number; hi: number }, band: { lo: number; hi: number }): boolean {
  return !(ci.lo > band.hi || ci.hi < band.lo)
}

function armSummary(arm: string, pairs: PairedScenario[], side: 'baseline' | 'candidate', ledgers: RunLedger[], repeats: number): ArmSummary {
...
function classify(b: ArmScenarioStats, c: ArmScenarioStats, repeats: number): PairClass {
  ...
  if (bAll && c.passes === 0) return 'regression'
  if (cAll && b.passes === 0) return 'improvement'
  if (b.passRate >= 0.5 && c.passRate <= 0.5 && c.passRate < b.passRate) return 'suspected'
  if (b.passRate < 0.5 && c.passRate < 0.5) return 'both-fail'
  return 'same'
}
...
export function buildReport(plan: RunPlan, ledgers: RunLedger[], options: ReportOptions = {}): Report {
  ...
  const candidates: CandidateReport[] = plan.candidates.map((cand) => {
    ...
    const floorStatus: CandidateReport['floor'] = floorOnFile === null ? 'missing' : drifted ? 'stale' : floorOnFile.scenarios < minScenarios ? 'thin' : 'ok'
    ...
    let regressionChance: number | null = null
    ...
    const gate: CandidateReport['gate'] = unsafe.length > 0 ? 'unsafe' : regressions.length > 0 ? 'regressions' : suspected.length > 0 ? 'suspect' : ran.length === 0 || incomplete.length === ran.length ? 'incomplete' : 'pass'
    ...
    else if (costReading === 'inconclusive' && floorBlocked) verdict = `Cost interval (${ciText}) excludes zero, but ${floorText}; a direction is not read until one is: dsh-eval run --baseline ${plan.baseline.name} --aa.${gains}`
    else if (costReading === 'inconclusive' && insideNoise) verdict = `Cost interval (${ciText}) reaches into the A/A noise band (${fmtPct(noiseFloor!.lo)} to ${fmtPct(noiseFloor!.hi)}) measured on this baseline by run ${noiseFloor!.runId}; not read as a real difference.${gains}`
  ...
  const readState = ledgers.filter(l => (l.evalFileReads?.length ?? 0) > 0)
  if (readState.length > 0) notes.push(`READ THE EVALUATION'S OWN FILES: ${readState.length} trial(s) opened files that belong to the evaluation rather than the task — ${[...new Set(readState.flatMap(l => l.evalFileReads!.map(r => r.split(' — ')[0]!)))].join(', ')} (first: ${readState[0]!.evalFileReads![0]!.slice(0, 140)}). The scenario directory holds every turn's prompt and the verifier, and the run directory holds the evidence; a verdict from a trial that read either may reflect what it found there rather than what it did. Read those traces before crediting anything to the component, and run under --sandbox docker, where neither is mounted.`)
