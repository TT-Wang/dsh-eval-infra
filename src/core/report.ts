...
export function buildReport(plan: RunPlan, ledgers: RunLedger[], options: ReportOptions = {}): Report {
  ...
  if (readState.length > 0) notes.push(`READ THE EVALUATION'S OWN FILES: ${readState.length} trial(s) opened files that belong to the evaluation rather than the task — ${[...new Set(readState.flatMap(l => l.harnessStateReads!.map(r => r.split(' — ')[0]!)))].join(', ')} (first: ${readState[0]!.harnessStateReads![0]!.slice(0, 140)}). The session store holds the trial's own transcript, the scenario directory holds every turn's prompt and the verifier, and the run directory holds the evidence; a verdict from a trial that read any of them may reflect what it found there rather than what it did. Read those traces before crediting anything to the component, and run under --sandbox docker, where none of the three is mounted.`)
