...
import { collectScenarios, launchRun, LaunchError, rebuildReport, resealRun, resolveArmPath, verifyRunIntegrity, type RunRequest } from '../core/orchestrate.js'
...
async function readBody(req: IncomingMessage): Promise<unknown> {
  ...
  if (text === '') return {}
  // A body is JSON or nothing: a cross-site page can send text/plain or a form without a preflight, never application/json.
  const type = String(req.headers['content-type'] ?? '').split(';')[0]!.trim().toLowerCase()
  if (type !== 'application/json') throw new RequestError(415, `request bodies must be application/json (got ${type || 'no content type'})`)
 ...
 */
export function crossSiteRequest(req: IncomingMessage): string | null {
  const site = String(req.headers['sec-fetch-site'] ?? '').toLowerCase()
  if (site !== '' && site !== 'same-origin' && site !== 'none') return `cross-site request refused (Sec-Fetch-Site: ${site})`
  const origin = req.headers['origin']
  if (typeof origin === 'string' && origin !== '' && origin !== 'null') {
    let host: string
    try { host = new URL(origin).host } catch { return `cross-site request refused (unparseable Origin ${origin})` }
    const own = String(req.headers['host'] ?? '')
    if (host.toLowerCase() !== own.toLowerCase()) return `cross-site request refused (Origin ${origin} is not this server ${own || '(no Host header)'})`
  } else if (typeof origin === 'string') return 'cross-site request refused (opaque Origin)'
  return null
}
...
export class EvalApp {
  ...
  async handle(req: IncomingMessage, res: ServerResponse, prefix = ''): Promise<boolean> {
    ...
    try {
      ...
      const status = error instanceof RequestError ? error.status : error instanceof LaunchError ? 400 : 500
      // A LaunchError is the user's request being wrong and says so in the response.
      // Anything else is a fault in here, and the browser only shows "500": print it,
      // with the stack, so whoever is running the server can see what actually broke.
      if (status === 500) process.stderr.write(`dsh-eval ui: ${req.method ?? 'GET'} ${path} failed\n${error instanceof Error ? (error.stack ?? error.message) : String(error)}\n`)
      json(res, status, { error: error instanceof Error ? error.message : String(error), code: error instanceof LaunchError ? error.code : error instanceof RequestError ? 'request' : 'internal' })
      return true
    }
  }
  ...
  private async api(req: IncomingMessage, res: ServerResponse, path: string, url: URL): Promise<void> {
    ...
    if (method !== 'GET' && method !== 'HEAD' && method !== 'OPTIONS') {
      const refused = crossSiteRequest(req)
      if (refused !== null) { json(res, 403, { error: refused }); return }
    }
    ...
    if (method === 'GET' && path === '/scenarios') {
      ...
      json(res, 200, { scenarios: scenarios.map(s => ({ name: s.name, dir: tilde(s.dir), meta: s.meta, turns: s.prompts.length, hasOracle: s.hasOracle, variants: s.variants?.length ?? 0, prompts: s.prompts })), invalid: invalid.map(i => ({ ...i, dir: tilde(i.dir) })) })
      return
    }
    ...
    if (method === 'POST' && path === '/scenarios') {
      const body = await readBody(req) as { name?: string; files?: Record<string, string> }
      const { addScenario } = await import('../core/intake.js')
      try {
        const added = await addScenario(project, body.name ?? '', body.files ?? {})
        json(res, 200, { ...added, dir: tilde(added.dir) })
      } catch (e) {
    ...
    if (method === 'GET' && path === '/arms') {
      ...
      const arms = files.map((f) => {
        ...
        try { const spec = loadArmFile(p); return { file: f, path: tilde(p), spec, text: readFileSync(p, 'utf8') } } catch (e) { return { file: f, path: tilde(p), error: e instanceof Error ? e.message : String(e), text: readFileSync(p, 'utf8') } }
      })
      json(res, 200, { dir: tilde(project.armsDir), arms })
      return
    }
    ...
    // Public benchmarks: the index (small, cached), one-click get (pulls the image), remove.
    if (method === 'GET' && path === '/bench') {
      const { ADAPTERS } = await import('../core/bench/index.js')
      const out = []
      for (const a of Object.values(ADAPTERS)) {
        const pool = a.poolDir(project)
    ...
    if (method === 'POST' && path === '/bench/get') {
      const body = await readBody(req) as { dataset?: string; task?: string }
      if (typeof body.dataset !== 'string' || typeof body.task !== 'string') { json(res, 400, { error: 'dataset and task are required' }); return }
      if (!validTaskId(body.task)) { json(res, 400, { error: 'task must be a single path segment: letters, digits, . _ -' }); return }
      const { adapterFor } = await import('../core/bench/index.js')
      const lines: string[] = []
      const r = await adapterFor(body.dataset).materialize(project, body.task, { log: (l) => lines.push(l) })
      json(res, 200, { dir: tilde(r.dir), task: r.task, taskHash: r.taskHash, log: lines })
      return
    }
    if (method === 'POST' && path === '/bench/rm') {
      const body = await readBody(req) as { dataset?: string; task?: string }
      if (typeof body.dataset !== 'string' || typeof body.task !== 'string') { json(res, 400, { error: 'dataset and task are required' }); return }
      if (!validTaskId(body.task)) { json(res, 400, { error: 'task must be a single path segment: letters, digits, . _ -' }); return }
      const { adapterFor } = await import('../core/bench/index.js')
      json(res, 200, { removed: adapterFor(body.dataset).remove(project, body.task) })
      return
    }
    ...
    if (method === 'GET' && path === '/rows') {
      ...
      if (armName === '') { json(res, 400, { error: 'no arm named' }); return }
      const spec = loadArmFile(resolveArmPath(project, armName))
      const prepared = await withPreviewArms(project, armsDir => prepareArms(spec, [], { evalHome: project.home, armsDir }))
    ...
    if (seg[0] === 'runs' && seg[1] !== undefined) {
      ...
      if (seg[2] === 'annotations') {
        ...
        if (method === 'PUT') {
          ...
          // Annotations are evidence the report derives from: a sealed run is sealed again over them.
