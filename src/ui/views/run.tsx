    ...
    <section class="run">
      ...
      <div class="card">
        ...
        <dl class="facts">
          ...
          <dt>evidence</dt><dd>{detail.integrity ? (detail.integrity.sealedAt ? <span>sealed {new Date(detail.integrity.sealedAt).toLocaleString()} · <code title="sha256 over every evidence file">{detail.integrity.evidenceSha?.slice(0, 16)}…</code> · {detail.integrity.ok ? <span class="cls same">verifies</span> : <span class="cls regression">FAILS verification</span>}{detail.integrity.changed.length ? ` · changed: ${detail.integrity.changed.slice(0, 3).join(', ')}` : ''}{detail.integrity.missing.length ? ` · missing: ${detail.integrity.missing.length}` : ''}{detail.integrity.reportReproduces === false ? ' · report does not re-derive from the ledgers' : ''}</span> : 'not sealed') : 'not sealed'}</dd>
    ...
    <div class={`verdict ${GRADE_TONE[c.grade]}`}>
      ...
      <div class="verdict-grid">
        <div class="cards">
          ...
          {c.judge && c.judge.lengthBalancedWinRate !== null && c.judge.lengthBalancedWinRate !== undefined && <Stat label="length-balanced win rate" a={`${(c.judge.lengthBalancedWinRate * 100).toFixed(0)}%`} b={`longer wins ${c.judge.longerWinsShare === null || c.judge.longerWinsShare === undefined ? '—' : (c.judge.longerWinsShare * 100).toFixed(0) + '%'}`} />}
          {c.rerun && <Stat label={`rerun validation · ${c.rerun.scenario}`} a={c.rerun.verdict} b={c.rerun.original ? `${c.rerun.original.failing} failed again ${c.rerun.failedAgain}/${c.rerun.reps} · same call ${c.rerun.sameCall}/${c.rerun.reps}` : `${c.rerun.failedAgain}/${c.rerun.reps} with one failing arm`} />}
