    ...
    <div class={`verdict ${GRADE_TONE[c.grade]}`}>
      ...
      <div class="verdict-grid">
        <div class="cards">
          ...
          {c.judge && <Stat label={`judge${(c.judge.models?.length ?? 1) > 1 ? ' panel' : ''} (${c.judge.model})`} a={`${c.judge.wins} / ${c.judge.losses} / ${c.judge.ties}`} b={`mid-p ${c.judge.midP.toFixed(2)} · orders disagree ${(c.judge.inconsistentShare * 100).toFixed(0)}%${(c.judge.models?.length ?? 1) > 1 ? ` · unanimous ${((c.judge.panelAgreement ?? 1) * 100).toFixed(0)}%` : ''}${c.judge.humanAgreement ? ` · κ ${c.judge.humanAgreement.kappa === null ? '—' : c.judge.humanAgreement.kappa.toFixed(2)}` : ''}`} />}
          {c.absolute && <Stat label={`absolute grades${c.absolute.baseline.n + c.absolute.candidate.n > 0 ? ' · PPI++' : ' · uncalibrated'}`} a={`${(c.absolute.baseline.estimate * 100).toFixed(0)}% → ${(c.absolute.candidate.estimate * 100).toFixed(0)}%`} b={`Δ ${(c.absolute.diff * 100).toFixed(0)} pp ± ${(c.absolute.diffSe * 100).toFixed(0)} · ${c.absolute.baseline.n + c.absolute.candidate.n} human labels`} />}
        </div>
