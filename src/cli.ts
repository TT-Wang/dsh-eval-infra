 ...
 */
async function cmdStatus(project: Project, args: Args): Promise<number> {
  const { projectStatus } = await import('./core/status.js')
  const st = await projectStatus(project)
  if (args.flags['json'] === true) { out(JSON.stringify(st, null, 2)); return 0 }
  const brief = (v: unknown): string => typeof v === 'number' ? (Number.isInteger(v) ? String(v) : v.toFixed(1))
    : Array.isArray(v) ? (v.length > 2 ? `${v.length}` : v.map(brief).join(','))
    : typeof v === 'object' && v !== null ? Object.entries(v as Record<string, unknown>).map(([k, x]) => `${k}:${brief(x)}`).join('/')
    : String(v)
  ...
  if (notYet.length > 0) {
    ...
    for (const [name, r] of notYet) out(`    ${name.padEnd(22)} ${(r.missing ?? []).map(m => m.code).join(', ')}`)
  }
...
function cmdVerify(project: Project, args: Args): number {
  const id = args.positional[0]
  if (id === undefined) { err('usage: dsh-eval verify <runId | run directory> [--json] [--key <public-key.pem>]'); return 3 }
  const keys = ([] as string[]).concat(...[args.flags['key']].flat().filter((k): k is string => typeof k === 'string').map(k => k.split(',')))
