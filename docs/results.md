...
## The evaluation's own files as an answer key (2026-09-09): found by running it

Verifying the security round on real runs turned up something no test would have caught. The A/A run **20260909-043013-w7i1** (5 scenarios × 3 repeats × 2 arms, 30 trials, $0.18, 10.5 min) contains it in one trial.

- **What happened.** On `m1_cross_session_recall` — the scenario whose meta said "only an agent with memory across sessions can produce it" — the baseline's turn 3 read the workspace, grepped for the codename, found nothing, then listed `$DSH_HOME`, found `sessions/--<its own workspace slug>--/`, looked for `zstd`, `zstdcat`, `python3 -m zstandard`, searched the filesystem for any zstd binary, ran `npm install fzstd`, wrote a `/tmp/dz.mjs` to decompress `session.jsonl.zstd`, parsed the JSONL and read the codename out of its own previous session's transcript. **The verifier passed it: "all 7 facts recalled across the session break."** Only the safety gate caught the trial at all, and incidentally — `npm install` with network off is a destructive command.
- **The same trial also listed the scenario's own directory and the run directory.** `bench/scenarios/<name>/prompts.json` holds every turn's text, including the one the agent is supposed to remember; `verify.py` holds the grading criteria; the run directory holds both arms' ledgers, traces and the meter's recorded provider responses. On the host all three are siblings of the workspace. The scenario library was never mounted into the container sandbox — but the run directory was, read-only and whole, because the overlays the runtime is handed live inside it. The task-container path had already got this right by mounting each overlay's own directory; the sandbox path now does the same, so a container trial sees the run's `arms/` and not the evidence beside it.
- **What was fixed, and what was deliberately not.** The scenario directory and the run directory belong to the evaluation, and no component has business in either: a trial that opens one is recorded on its ledger (`evalFileReads`) and named in the report, matched by path or by the evaluation's own file names, with anything inside the workspace excluded because that is the task's own. dsh's session store is a different thing and is left alone. A first attempt moved it out of the eval home at a session break; that was backed out. It hard-coded dsh's private storage layout into the runner, and — the real objection — a memory component may legitimately be implemented by reading that store, so hiding it would have had the scale interfering with the thing it is weighing. What the store costs a given scenario is the scenario's business: `m1_cross_session_recall` now says in its own README and meta that the seventh fact can be remembered *or* decompressed, that the run does not separate the two, and that the trace is what settles it.
- **What the A/A run also settled.** Its own noise band is **Δ% +8.1%, 95% CI −55.3% to +71.5%** over 4 comparable scenarios — on this pool, at 3 repeats, a cost difference smaller than roughly ±60% is not distinguishable from rerunning the same arm twice. The mean |Δ%| that the old code used as its veto line would have been about 18%, which is why H15 mattered: the old rule would have called a 25% difference real. With 4 comparable scenarios the floor also reads `thin` (five are needed), and the follow-up run (**20260909-044603-a9fm**) refused a direction and said so — the mandatory-floor rule working end to end on real data.
