...
Verifying the security round on real runs turned up a validity hole the review had pointed at but that no test could have caught. The A/A run **20260909-043013-w7i1** (5 scenarios × 3 repeats × 2 arms, 30 trials, $0.18, 10.5 min) contains the whole story in one trial.
...
- **How far it went.** The same trial also listed the scenario's own directory and the run directory. `bench/scenarios/<name>/prompts.json` holds every turn's text, including the one the agent is supposed to remember; `verify.py` holds the grading criteria. On the host all three — session store, scenario directory, run directory — are siblings of the workspace. Under `--sandbox docker` none of them is mounted.
- **The fix.** A session ended on purpose now has its transcript moved out of the eval home before the next session starts, together with the plaintext cache that keeps its first prompt (`stashSessionStore`, the same move-and-restore pattern as `.truth`); it is merged back when the trial ends. What cannot be moved — the live session's file, the scenario library and the run directory, all shared by concurrent trials — is detected instead: `harnessStateReads` records the tool calls on the ledger, by literal path, through `$DSH_HOME`, or by dsh's own file names, and the report says the verdict may reflect what the trial read rather than what it did. Detection, not prevention: a flag, not a wall.
- **Before and after, same scenario, same code path.** Before: verifier "pass — all 7 facts recalled". After (run **20260909-044603-a9fm**, both arms): `handoff.md missing/wrong: ['codename'] (6/7 recalled)` — exactly the six facts that live in the workspace by design, and the seventh, which lives only in a conversation, gone. The store is restored intact afterwards: both trials' two sessions, both transcripts, 4/4 cache entries.
- **What the A/A run also settled.** Its own noise band is **Δ% +8.1%, 95% CI −55.3% to +71.5%** over 4 comparable scenarios — on this pool, at 3 repeats, a cost difference smaller than roughly ±60% is not distinguishable from rerunning the same arm twice. The mean |Δ%| that the old code used as its veto line would have been about 18%, which is why H15 mattered: the old rule would have called a 25% difference real. With 4 comparable scenarios the floor also reads `thin` (five are needed), and the follow-up run refused a direction and said so — the mandatory-floor rule working end to end on real data.
